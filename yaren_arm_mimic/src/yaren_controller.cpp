...
DualArmTrajectoryController::DualArmTrajectoryController() : Node("body_trajectory_controller") {
    // Límites físicos del robot (rad) — del URDF real
    joint_limits_["joint_2"]  = std::make_pair(-0.5235f, 0.5235f);
    joint_limits_["joint_5"]  = std::make_pair(-0.7853f, 1.5708f);
    joint_limits_["joint_6"]  = std::make_pair( 0.0f,    1.0472f);
    joint_limits_["joint_7"]  = std::make_pair(-0.7853f, 0.7853f);
    joint_limits_["joint_8"]  = std::make_pair( 0.1745f, 1.5708f);
    joint_limits_["joint_9"]  = std::make_pair(-0.7853f, 1.5708f);
    joint_limits_["joint_10"] = std::make_pair( 0.0f,    1.0472f);
    joint_limits_["joint_11"] = std::make_pair(-0.7853f, 0.7853f);
...
// ─────────────────────────────────────────────────────────────────────────────
//  Mapeo ángulos humanos → posición del joint del robot
//
//  Rango humano observado (grados):
//    Hombro ZY: brazo pegado al cuerpo ≈ 0°, brazo arriba ≈ +160°
//    Hombro YX: neutro ≈ 0°, adelante ≈ +90°, atrás ≈ -90°
//    Codo   ZY: brazo recto ≈ 0°, doblado ≈ -90°
//    Codo   YX: rotación muñeca ≈ ±90°
// ─────────────────────────────────────────────────────────────────────────────
std::map<std::string, float>
DualArmTrajectoryController::processArmData(const std::array<float, 4>& angles,
    ...
    // angles[0] = hombro ZY  → joint 5/9  (elevación lateral)
    // Humano: 0° (abajo) .. +160° (arriba) → robot: -0.7853 .. 1.5708 rad
    float j_shoulder_zy = mapRange(angles[0], -20.0f, 160.0f,
                                   joint_limits_[arm_joints[0]].first,
                                   joint_limits_[arm_joints[0]].second);

    // angles[1] = hombro YX  → joint 6/10  (flexión/extensión)
    // Humano: -90° (atrás) .. +90° (adelante) → robot: 0 .. 1.0472 rad
    ...
    // angles[2] = codo ZY  → joint 7/11  (flexión codo)
    // Humano: 0° (recto) .. -90° (doblado) → robot: -0.7853 .. 0.7853 rad
    float j_elbow_zy = mapRange(-angles[2], 0.0f, 90.0f,
                                0.0f,
                                joint_limits_[arm_joints[2]].second);

    // angles[3] = codo YX  → joint 8/12  (rotación)
    // Humano: ±150° → robot: 0.1745 .. 1.5708 rad
