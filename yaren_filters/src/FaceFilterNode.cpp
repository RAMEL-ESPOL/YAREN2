...
struct LangStrings {
    ...
    std::string apply_btn;
    std::string categories_title;
    std::string disabled_txt;
    std::string active_txt;
    std::string of_txt;
    bool is_eng;
...
static LangStrings get_strings(bool is_english) {
    LangStrings s;
    s.is_eng = is_english;
    if (is_english) {
        s.menu_title = "Configure your accessories";
        s.apply_btn = "Apply";
        s.categories_title = "CATEGORIES";
        s.disabled_txt = "disabled";
        s.active_txt = " active";
        s.of_txt = " of ";
    } else {
        s.menu_title = "Configura tus accesorios";
        s.apply_btn = "Aplicar";
        s.categories_title = "CATEGORIAS";
        s.disabled_txt = "desactivado";
        s.active_txt = " activo";
        s.of_txt = " de ";
    }
    return s;
...
const cv::Scalar BG_MAIN(20, 13, 13);
const cv::Scalar BG_HEADER(31, 19, 19);
const cv::Scalar BORDER_COLOR(58, 42, 42);
const cv::Scalar TEXT_LIGHT(240, 226, 226);
const cv::Scalar TEXT_MUTED(160, 124, 124);
const cv::Scalar COLOR_PURPLE(250, 139, 167);
const cv::Scalar COLOR_GREEN(128, 222, 74);
...
static void draw_menu(cv::Mat& canvas, const std::vector<CatState>& states, MenuMouseState* ms, bool is_english)
{
    ...
    static const cv::Scalar ICON_BG[]  = {
        {30,  60, 160}, {20, 120, 200}, {40, 160,  80}, {160, 40, 120}, {100,  30, 180}
    };
    ...
    for (int i = 0; i < (int)CATS.size(); ++i) {
        ...
        if (st.enabled) {
            ...
            char buf[32];
            snprintf(buf, sizeof(buf), "%d%s%d", st.index+1, txt.of_txt.c_str(), st.max_idx);
    ...
    for (int i = 0; i < (int)CATS.size(); ++i) {
        ...
        std::string title = txt.cat_label(cat.id) + txt.active_txt;
        std::transform(title.begin(), title.end(), title.begin(), ::toupper);
        cv::putText(right_canvas, title, {card.x+12, card.y+18}, cv::FONT_HERSHEY_SIMPLEX, 0.33, TEXT_MUTED, 1, cv::LINE_AA);
        ...
        if (st.index >= 0 && st.index < (int)st.previews.size() && !st.previews[st.index].empty()) {
            ...
            cv::Mat ts;
            cv::resize(thumb, ts, cv::Size((int)(thumb.cols*sc),(int)(thumb.rows*sc)), 0, 0, cv::INTER_AREA);
        ...
        cv::putText(right_canvas, "<", {bprev.x+10,bprev.y+22}, cv::FONT_HERSHEY_SIMPLEX, 0.6, arr_col, 2, cv::LINE_AA);
        cv::putText(right_canvas, ">", {bnext.x+10,bnext.y+22}, cv::FONT_HERSHEY_SIMPLEX, 0.6, arr_col, 2, cv::LINE_AA);
        ...
        cv::putText(right_canvas, ibuf, {card.x+(card.width-tw)/2, card.y+154}, cv::FONT_HERSHEY_SIMPLEX, 0.45, cv::Scalar(144,144,176), 1, cv::LINE_AA);
    ...
    for (int i = 0; i < (int)CATS.size(); ++i) {
        ...
        cv::putText(canvas, txt.cat_label(CATS[i].id), {pill.x+18, pill.y+18}, cv::FONT_HERSHEY_SIMPLEX, 0.33, ptxt, 1, cv::LINE_AA);
        px += pill.width + 6;
    }
...
//  show_menu()
// ================================================================
static bool show_menu(std::vector<CatState>& states, std::atomic<bool>& is_english)
{
    ...
    while (!ms.apply_hit && !ms.close_hit) {
        draw_menu(canvas, states, &ms, is_english.load());
...
public:
    ...
    FaceFilterNode() : Node("face_filter_node"), running_(true)
    {
        ...
        // --- Suscripción dinámica al idioma ---
        auto lang_qos = rclcpp::QoS(rclcpp::KeepLast(1)).transient_local();
        language_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            "/yaren/is_english", lang_qos,
            [this](const std_msgs::msg::Bool::SharedPtr msg) {
                is_english_ = msg->data;
                RCLCPP_INFO(this->get_logger(), "Idioma recibido en filtros: %s", is_english_ ? "English" : "Español");
            });
...
int main(int argc, char** argv)
{
    ...
    // Esperar un momento a que el nodo reciba el mensaje del idioma publicado por face_screen
    ...
    while (rclcpp::ok()) {
        // Se envía la referencia atómica para que el menú se actualice si el idioma cambia
        bool applied = show_menu(states, node->is_english_);
