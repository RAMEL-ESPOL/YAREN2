...
{
    std::string yarenShareDir = ament_index_cpp::get_package_share_directory("yaren_face_screen");

    eyesOpenImg = cv::imread(yarenShareDir + "/imgs/open_eyes.png", cv::IMREAD_UNCHANGED);
    eyesClosedImg = cv::imread(yarenShareDir + "/imgs/close_eyes.png", cv::IMREAD_UNCHANGED);
    mouthClosedImg = cv::imread(yarenShareDir + "/imgs/close_mouth.png", cv::IMREAD_UNCHANGED);
    mouthOpenImg = cv::imread(yarenShareDir + "/imgs/open_mouth.png", cv::IMREAD_UNCHANGED);
    
    loadFrames(yarenShareDir + "/imgs_transition/blinking_frames", eyesFrames);
    loadFrames(yarenShareDir + "/imgs_transition/talking", mouthFrames);
...
void VideoSynchronizer::displayFrame(const cv::Mat& frame)
{
    if (!frame.empty())
    {
        cv::Mat displayFrame;
